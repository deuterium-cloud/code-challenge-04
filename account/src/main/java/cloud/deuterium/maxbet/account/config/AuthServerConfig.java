...
public class AuthServerConfig {
    ...
    public RegisteredClientRepository registeredClientRepository(SecurityProperties properties) {
        ...
        RegisteredClient publicClient = RegisteredClient
                ...
                .clientId(properties.getName())
                .clientAuthenticationMethod(ClientAuthenticationMethod.NONE)
                .authorizationGrantType(AuthorizationGrantType.AUTHORIZATION_CODE)
                .redirectUri(properties.getRedirectUri())
                .postLogoutRedirectUri(properties.getLogoutRedirectUri())
    ...
    @Bean
    public OAuth2TokenCustomizer<JwtEncodingContext> jwtCustomizer(@Value("${app.jwt.name}") String appName) {
        return context -> {
            WoGUserDetails userDetails = (WoGUserDetails) context.getPrincipal().getPrincipal();
            Account account = userDetails.getAccount();

            context.getClaims()
                    .claim("app", appName)
                    .claim("roles", Set.of(account.getRole().name()))
